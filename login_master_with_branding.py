...
/* Hero section */
.hero-section {
    background: linear-gradient(135deg, #0f4c75 0%, #00b894 100%);
    padding: 2rem;
    border-radius: 15px;
    color: white;
    text-align: center;
...
/* Card button styling - ORIGINAL HEIGHT, ONLY LINE SPACING REDUCED */
.stButton > button {
    width: 100%;
    ...
    font-size: 0.95rem;
    line-height: 1.45;  /* ONLY THIS REDUCED (was 1.7) */
...
/* Card content formatting - TIGHTER LINE SPACING */
.stButton > button p {
    margin: 0.5rem 0;  /* ONLY THIS REDUCED (was 0.8rem) */
}

/* Gradient backgrounds for different card types */
[data-testid="column"]:nth-child(1) .stButton > button {
    background: linear-gradient(135deg, rgba(30, 58, 138, 0.05) 0%, rgba(0, 184, 148, 0.05) 100%);
}

[data-testid="column"]:nth-child(2) .stButton > button {
    background: linear-gradient(135deg, rgba(255, 255, 0, 0.05) 0%, rgba(0, 184, 148, 0.05) 100%);
}

[data-testid="column"]:nth-child(3) .stButton > button {
    background: linear-gradient(135deg, rgba(59, 130, 246, 0.05) 0%, rgba(0, 184, 148, 0.05) 100%);
}
...
def render_agency_verification_for_company():
    ...
    st.markdown("---")
    st.info("💡 Enter the agency code provided by your placement agency")
...
def render_login():
    ...
    if not st.session_state.get('show_register', False):
        # Show login form only if NOT in registration mode
...
def main():
    ...
    if st.session_state.get("logged_in", False):
        ...
        if page == "landing":
            ...
            #col_left, col_center, col_right = st.columns([0.6, 0.8, 0.6])
            #with col_center:
            #    col1, col2, col3, col4 = st.columns(4)

            # Hero section
            st.markdown("""
                <div class="hero-section" style="max-width: 800px; margin-left: auto; margin-right: auto;">
            ...
            # Clickable card buttons (Pure Streamlit - No HTML tricks!)
            col_left, col_center, col_right = st.columns([0.6, 0.8, 0.6])
            ...
            with col_center:
                ...
                with col1:
                    if st.button(
                        "🏢\n**Register as Agency**\n\nOwn a placement agency? Get your branded dashboard, candidate management, company portal & auto-reports.",
                        key="card_agency",
                ...
                with col2:
                    if st.button(
                        "👤\n**Already Registered?**\n\nLogin to your agency dashboard with your Portal Code (AG001).",
                        key="card_login",
                        use_container_width=True
                    ):
                        st.session_state.page = "accept_tc"
                ...
                with col3:
                    if st.button(
                        "🏭\n**Register as Company**\n\nPost jobs, manage interviews, track candidates from verified agencies.",
                        key="card_company",
                        use_container_width=True
                    ):
                        st.session_state.page = "accept_tc"
                ...
                with col4:
                    if st.button(
                        "👨‍💼\n**Register as Applicant**\n\nUpload resume, apply to jobs, get interview calls from verified agencies.",
                        key="card_applicant",
                        use_container_width=True
                    ):
                        st.session_state.page = "accept_tc"
