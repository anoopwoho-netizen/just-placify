...
# IMPORTS
# ==================================================== 
from ui_components import apply_global_styling, render_kpi_card, render_enhanced_dataframe, render_empty_state, render_breadcrumb
...
def apply_custom_sidebar_css():
    """Apply complete sidebar styling"""
    css = """
    <style>
...
def render_sidebar():
    ...
    if st.session_state.get('logo_url', ''):
        st.sidebar.image(st.session_state.logo_url, width=150)
    ...
    try:
        ...
        pass  # Sheet nahi hai to ignore karo

    
    agency_name = st.session_state.get('agency_name', 'Placement Agency System')
    
    st.sidebar.markdown(f"""
    <div style="padding: 10px 0; border-bottom: 1px solid rgba(147, 51, 234, 0.2); margin-bottom: 15px;">
        <h1 style="margin: 0; font-size: 20px; color: #ffffff; font-weight: 700;">{agency_name}</h1>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.get("full_name"):
        ...
        <div style="margin-bottom: 8px;">
            <div style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.1), rgba(34, 197, 94, 0.05));
                border: 1px solid rgba(34, 197, 94, 0.3); padding: 10px; border-radius: 8px;">
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                    <div style="width: 16px; height: 16px; background-color: #22c55e; border-radius: 50%;
                        display: flex; align-items: center; justify-content: center;">
                        <span style="color: white; font-size: 10px; font-weight: bold;">✓</span>
                    </div>
                    <span style="color: #ffffff; font-weight: 600; font-size: 13px;">{full_name}</span>
                </div>
                <div style="border-top: 1px solid rgba(255,255,255,0.1); padding-top: 8px; margin-top: 8px;">
                    <p style="margin: 0 0 4px 0; color: #10b981; font-size: 9px; text-transform: uppercase;">Role</p>
                    <p style="margin: 0 0 8px 0; color: #ffffff; font-weight: 600; font-size: 12px;">{role}</p>
                    <p style="margin: 0 0 4px 0; color: #3b82f6; font-size: 9px; text-transform: uppercase;">Email</p>
                    <p style="margin: 0; color: #ffffff; font-size: 11px; word-break: break-all;">{email}</p>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def admin_dashboard():
    ...
    with col1:
        render_kpi_card(title="Total Companies", value=len(companies_df), icon="🏢")
    with col2:
        render_kpi_card(title="Total Vacancies", value=len(vacancies_df), icon="💼")
    with col3:
        render_kpi_card(title="Total Candidates", value=len(candidates_df), icon="👥")
    with col4:
        ...
        render_kpi_card(title="Selections", value=selected_count, icon="🎉")
...
def admin_advanced_filtering():
    ...
    st.markdown("Filter with 70+ columns available!")
...
def main():
    ...
    if user_role == "super_admin":
        if menu_choice == "Dashboard":
            ...
            st.info("⚠️ Agency Management feature under development")
